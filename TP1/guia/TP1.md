...
# TP1: Introducción a programación en Bash
...
## Uso de programación en bioinformática
En bioinformática, la mayor parte del trabajo consiste en procesar grandes volúmenes de datos. Hacer esto a mano o con programas que tienen ventanas (interfaces gráficas) puede ser lento, tedioso y difícil de repetir.

Aprender a usar la terminal y escribir scripts te permite:

- **Automatizar tareas repetitivas**: Si tenés que procesar 10 archivos, lo hacés con un bucle. Si tenés 1000, el mismo bucle funciona igual. No importa la cantidad.

- **Trabajar con datos masivos**: Los archivos de secuenciación pueden tener cientos de miles de líneas. Abrirlos con un editor de texto es imposible. Desde la terminal podés filtrarlos, buscar patrones y extraer información en segundos.

- **Hacer reproducible tu trabajo**: Un script documenta exactamente qué hiciste con los datos. Si alguien quiere repetir tu análisis, solo tiene que ejecutar el mismo script. No hay pasos olvidados ni cosas hechas "a mano" que no se puedan replicar.

En este tutorial, vas a experimentar esto en la práctica: vamos a procesar varios genomas completos usando solo comandos y un script. 
...
a) ¿Cuántas mutaciones están documentadas en la tabla? ¿En qué posiciones de la proteína ocurren?

b) Identificá las **dos mutaciones** que, según la descripción, **aumentan la eficiencia catalítica de la enzima y generan resistencia a ceftazidima**. Anotá:
...
c) Según la tabla, ¿estas dos mutaciones actúan de forma independiente o **necesitan estar juntas** para generar el efecto? ¿Qué frase de la tabla te indica eso?
</div>
...
<strong>Introducción a Linux / Ubuntu</strong><br>
...
<strong>Introducción a Bash</strong><br>
...
**Nota:** Si estás en **Linux o macOS**, ya tenés una terminal nativa. Solo buscá "Terminal" en el menú de aplicaciones o usá el atajo de teclado correspondiente (Ctrl+Alt+T en Linux, o Comando+Espacio en macOS).

Se abrirá una ventana de terminal lista para usar!
...
<div style="border-left: 6px solid  #555; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Comandos básicos de Bash</strong><br>
...
<div style="border-left: 6px solid  #dc3545; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Errores más comunes en Bash</strong><br>
...
<div style="border-left: 6px solid  #555; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Estructura de directorios de Ubuntu</strong><br>
...
<div style="border-left: 6px solid  #dc3545; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Errores más comunes en para `cd`</strong><br>
...
<div style="border-left: 6px solid  #555; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Opciones más comunes</strong><br>
...
Los ejemplos que aparecen a continuación sirven solo para entender la sintaxis, ¡no los corran! El código que deben ejecutar está indicado en la sección Ejercicios.
...
<div style="border-left: 6px solid  #555; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Manuales en Linux</strong><br>

En Linux, los **manuales** (o páginas `man`) son la documentación oficial integrada en el sistema para casi todos los comandos, programas y utilidades disponibles. Son la fuente más confiable y completa para entender cómo funciona una herramienta, qué opciones tiene y cómo usarla correctamente.

### Cómo usar el comando `man`

Para acceder al manual de un comando, simplemente escribís `man` seguido del nombre del comando:

    man wget

Esto abre la página del manual de `wget` con su sintaxis, descripción de opciones, ejemplos y más.

### Estructura de una página de manual

Las páginas de manual suelen tener esta estructura estándar:

- **NAME**: nombre del comando y una breve descripción de lo que hace.
- **SYNOPSIS**: muestra la sintaxis básica del comando, indicando qué argumentos y opciones puede recibir.
- **DESCRIPTION**: explica en detalle qué hace el comando y cómo funciona.
- **OPTIONS**: lista todas las opciones o banderas (flags) que acepta el comando, con su descripción.
- **EXAMPLES**: ejemplos prácticos de uso (no siempre está presente, pero es muy común).
- **SEE ALSO**: referencias a otros comandos o manuales relacionados.

### Navegación dentro del manual

Una vez que abriste el manual con `man`, podés navegar de la siguiente forma:

- **Flechas / Page Up / Page Down**: moverse arriba y abajo.
- **/palabra**: buscar una palabra dentro del manual (presioná `n` para ir a la siguiente coincidencia).
- **q**: salir del manual y volver a la terminal.
...
<div style="border-left: 6px solid  #dc3545; background-color: #f5f5f5; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
<strong>Errores más comunes en para `head` y `tail`</strong><br>
...
---
### Paso 0: Diagramar el script

Vamos a pensar que pasos son necesarios llevar a cabo para realizar este análisis. 
...
```bash
    chmod +x ~/tp_resistencia/scripts/analisis_resistencia.sh
```

**Verificación:** Ejecutá `ls -l ~/tp_resistencia/scripts/analisis_resistencia.sh` y verificá que los permisos incluyan `-rwxr-xr-x` (la `x` indica que es ejecutable).
...
| Objetivo | Se cumplió? |
| :--- | :--- |
|1. Familiaarizarse con el uso de la terminal y sus comandos básicos para navegar, crear directorios y manipular archivos. | Si / No |
|2. Obtener y procesar datos biológicos reales desde bases de datos públicas usando herramientas de línea de comandos.  | Si / No |
|3. Automatizar tareas utilizando scripts, para procesar múltiples archivos.  | Si / No |
